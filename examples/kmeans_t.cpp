...
int main(int ac, char *argv[]) {
	...
	if (ac<2) {
		...
		printf("  The number of prototype buckets (K), the amount of \n");
		printf("  training vectors and the alpha value can be adjusted.\n");
	...
	DVector v(4), w(4);
	KMeans cluster(k, alp, dis, exp);
	
	printf("KMeans test routine with %i bucket-vectors consisting of 4 elements\n",k);
	printf("\n(1)  initializing the bucket-vectors\n\n");
	for (vei_t j=0; j<k; j++)
	{
		v.set_comp(((u_8b_max)*((double)(rand())/(double)(RAND_MAX+1))), U8B_TYPE, 0);
		 
		if (floor(2.0*rand()/(RAND_MAX+1.0))==1)
			v.set_comp(((s_32b_max)*((double)(rand())/(double)(RAND_MAX+1))),  S32B_TYPE,1); 
		else
			v.set_comp(((s_32b_min)*((double)(rand())/(double)(RAND_MAX+1))),  S32B_TYPE,1);
			 
		if (floor(2.0*rand()/(RAND_MAX+1.0))==1)
			v.set_comp(((f_32b_max/1)*((double)(rand())/(double)(RAND_MAX+1))), F32B_TYPE,2); 
		else
			v.set_comp(((f_32b_min/1)*((double)(rand())/(double)(RAND_MAX+1))), F32B_TYPE,2);
			
		if (floor(2.0*rand()/(RAND_MAX+1.0))==1)	
  			v.set_comp(((f_64b_max/2)*((double)(rand())/(double)(RAND_MAX+1))),  F64B_TYPE,3); 
		else
			v.set_comp(((f_64b_min/2)*((double)(rand())/(double)(RAND_MAX+1))),  F64B_TYPE,3);
