...
class KNN {
	private:
		...
		vei_t *kclasses;     
		f_64b *kdist;
		f_64b wincl;   // last overall winner's weight
		f_64b allcl;   // sum of all winner's weight
	...
	public:
		...
		vei_t access(VectorPoCl& datav, char dist=DIS_EUCL, vei_t expo=2);
		vei_t get_k_dis(VectorPoCl& datav, char dist=DIS_EUCL, vei_t expo=2);
		vei_t get_class(vei_t i) { return (i<knn)?kclasses[i]:-1; };
		f_64b get_dist(vei_t i) { return (i<knn)?kdist[i]:-1; };
		f_64b get_conf() { return (wincl/allcl); };
